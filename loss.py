...
# Cell
def batch_hard_triplet_loss(k, num_subset_sample, labels, embeddings, margin, squared=False, device='cpu'):
    """Build the triplet loss over a batch of embeddings.
    ...
    # hardest_positive_dist, _ = anchor_positive_dist.max(1, keepdim=True)
    # shape (batch_size, k)
    hardest_positive_dist, _ = anchor_positive_dist.topk(k, dim=1, largest=True, sorted=True)
    ...
    # hardest_negative_dist, _ = anchor_negative_dist.min(1, keepdim=True)
    # shape (batch_size, k)
    hardest_negative_dist, _ = anchor_negative_dist.topk(k, dim=1, largest=False, sorted=True)
